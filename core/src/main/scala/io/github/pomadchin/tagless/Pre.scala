...
/**
 * Newtype for `[F[_], A] =>> F[Unit]`. An algebra `U[Pre[F, _]]` is an algebra which translates all actions to `F[Unit]`. This is useful to represent
 * actions preceding main logic.
 */
object Pre extends PreInstances {
  ...
  def asMid[F[_]: Apply]: Pre[F, _] ~> Mid[F, _] = FunctionKLift[Pre[F, _], Mid[F, _]](p => fa => p.value *> fa)
  ...
  implicit final class TofuPreAlgebraSyntax[F[_], U[f[_]]](private val self: U[T[F, _]]) extends AnyVal {
    def attach(alg: U[F])(implicit U: ApplyK[U], F: Apply[F]): U[F] =
      U.map2K(self, alg)(FunctionKLift[Tuple2K[T[F, _], F, _], F](t2k => t2k.first.value *> t2k.second))
  }
}
...
private class PreAlgebraSemigroup[F[_], U[f[_]]](implicit F: Apply[F], U: ApplyK[U]) extends Semigroup[U[Pre[F, _]]] {
  def combine(x: U[Pre[F, _]], y: U[Pre[F, _]]): U[Pre[F, _]] =
    U.map2K(x, y)(FunctionKLift[Tuple2K[Pre[F, _], Pre[F, _], _], Pre[F, _]](t2k => Pre.apply(t2k.first.value *> t2k.second.value)))
}
